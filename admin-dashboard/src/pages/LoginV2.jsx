...
export default function LoginV2({ onLoginSuccess }) {
  ...
  return (
    <div
      className="min-h-screen bg-bg-primary flex items-center justify-center p-4 relative overflow-hidden font-sans transition-colors duration-500"
    >
      {/* Background decorations */}
      <div className="absolute -top-20 -left-20 w-64 h-64 rounded-full bg-accent-purple/10 blur-3xl animate-pulse" />
      <div className="absolute -bottom-20 -right-20 w-64 h-64 rounded-full bg-accent-cyan/10 blur-3xl animate-pulse" style={{ animationDelay: '1s' }} />
      ...
      >
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-base font-bold text-text-primary tracking-tight">VISION ADMIN</h1>
            <p className="text-[9px] text-text-secondary font-semibold tracking-widest opacity-50 uppercase">Security Portal</p>
          </div>
        </div>
        ...
        <p className="text-text-secondary mb-6 text-xs font-medium">
          {isRegisterMode ? "Tạo tài khoản quản trị mới" : "Truy cập hệ thống điều hành AI"}
        </p>

        {/* Email Field */}
        <div className="space-y-1.5 mb-4">
          <label htmlFor="email" className="block text-text-secondary text-[10px] font-bold uppercase tracking-wider ml-1">
            Email
          </label>
          ...
          {errors.email && (
            <p className="text-accent-red text-[10px] mt-1 ml-1 font-bold">{errors.email.message}</p>
          )}
        </div>

        {/* Password Field */}
        <div className="space-y-1.5 mb-4">
          <label htmlFor="password" className="block text-text-secondary text-[10px] font-bold uppercase tracking-wider ml-1">
            Mật khẩu
          </label>
          <div className="relative">
            <input
          ...
          {errors.password && (
            <p className="text-accent-red text-[10px] mt-1 ml-1 font-bold">{errors.password.message}</p>
          )}
        </div>
        ...
        {isRegisterMode && (
          <div className="space-y-1.5 mb-6">
            <label htmlFor="confirmPassword" className="block text-text-secondary text-[10px] font-bold uppercase tracking-wider ml-1">
              Xác nhận mật khẩu
            </label>
            <div className="relative">
              <input
            ...
            {errors.confirmPassword && (
              <p className="text-accent-red text-[10px] mt-1 ml-1 font-bold">{errors.confirmPassword.message}</p>
            )}
          </div>
        )}
        ...
        >
          {submitting ? (
            <>
              <span className="loader-ring border-white/30 border-t-white" style={{ width: 14, height: 14 }} />
        ...
        <div className="mt-6 text-center">
          ...
          >
            {isRegisterMode ? (
              <>Đã có tài khoản? <span className="text-accent-purple group-hover:text-accent-cyan transition-colors underline underline-offset-4 decoration-2">Đăng nhập</span></>
            ) : (
              <>Chưa có tài khoản? <span className="text-accent-purple group-hover:text-accent-cyan transition-colors underline underline-offset-4 decoration-2">Yêu cầu quyền</span></>
            )}
          </button>
        </div>
      </form>
