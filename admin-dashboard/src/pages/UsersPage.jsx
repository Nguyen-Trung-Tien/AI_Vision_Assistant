    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        message={
          confirm?.type === "lock" ? (
            ...
            <div className="text-sm">
              ...
              sẽ bị xoá vĩnh viễn khỏi cơ sở dữ liệu.
            </div>
          )
        }
        confirmLabel={
          ...
          <div className="flex gap-2">
            {isSuperAdmin && (
              <button
                ...
                className="flex-1 sm:flex-none flex items-center justify-center gap-2 min-h-[40px] px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-[11px] font-bold uppercase tracking-widest transition-all active:scale-95 shadow-lg shadow-indigo-600/10"
              >
            ...
            <button
              ...
              disabled={exporting}
              className="flex-1 sm:flex-none flex items-center justify-center gap-2 min-h-[40px] px-4 py-2 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 text-[11px] font-bold uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50"
            >
              <Download
      ...
      >
        {users.map((user) => (
          <tr
            key={user.id}
            className="hover:bg-white/[0.03] transition-colors group border-b border-border-primary last:border-0"
          >
            <td className="px-4 py-3">
              <div className="flex items-center gap-4">
                <div className="relative">
                  <Avatar
                    email={user.email}
                    className="w-9 h-9 rounded-2xl border-2 border-border-primary group-hover:border-indigo-500/50 transition-colors"
                  />
                  {!user.is_active && (
                ...
                <div className="min-w-0">
                  <p className="text-sm font-black text-text-primary truncate flex items-center gap-2">
                    {user.email}
                    {user.email === myEmail && (
                      <span className="px-1.5 py-0.5 rounded bg-white/10 text-[8px] uppercase tracking-widest text-text-secondary">
                        Tôi
                      </span>
                    )}
                  </p>
            ...
            <td className="px-4 py-3">
              <div className="flex justify-end items-center gap-2">
                ...
                <button
                  ...
                  className="w-9 h-9 rounded-xl bg-white/5 border border-white/5 flex items-center justify-center text-text-secondary hover:text-cyan-400 hover:border-cyan-400/30 transition-all"
                ...
                {isSuperAdmin && user.email !== myEmail && (
                  <>
                    ...
                    <button
                      ...
                      className={`w-9 h-9 rounded-xl bg-white/5 border border-white/5 flex items-center justify-center transition-all ${user.is_active ? "text-text-secondary hover:text-orange-400 hover:border-orange-400/30" : "text-green-500 hover:bg-green-500/10 border-green-500/20"}`}
                      title={
                    ...
                    <button
                      ...
                      className="w-9 h-9 rounded-xl bg-white/5 border border-white/5 flex items-center justify-center text-text-secondary hover:text-red-500 hover:border-red-500/30 transition-all"
                      title="Xoá vĩnh viễn"
                    >
