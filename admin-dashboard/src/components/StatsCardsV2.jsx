...
function SkeletonCard() {
  return <div className="bg-bg-card rounded-2xl p-7 border border-accent-purple/10 h-36 loading-shimmer" />;
}
...
export default function StatsCardsV2() {
  ...
  const cards = [
    ...
    },
    {
      ...
      icon: "%",
      value: data.avgConfidence ? `${(data.avgConfidence * 100).toFixed(1)}%` : "N/A",
      label: "Độ tin cậy TB",
    },
  ];
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
      {cards.map((card) => (
        ...
        >
          ...
          <div className={`w-12 h-12 rounded-xl ${card.iconBg} flex items-center justify-center text-sm font-bold mb-4`}>
            {card.icon}
          </div>
          <div className={`text-4xl font-extrabold tracking-tight mb-1 ${card.valueColor}`}>{card.value}</div>
