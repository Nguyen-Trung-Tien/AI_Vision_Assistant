...
export class FaceService {
  ...
  async registerFace(userId: string, name: string, frameData: string) {
    this.logger.log(`Received face registration request for user ${userId}, name: ${name}`);
    ...
    try {
      ...
      this.logger.log(`Task GET_FACE_ENCODING for ${name} emitted to RabbitMQ queue 'ai_tasks_queue'`);
    } catch (error) {
      this.logger.error(`Error emitting face registration task: ${error.message}`);
      throw error;
    }

    this.logger.log(`Successfully initiated registration process for ${name} (User: ${userId})`);
    return { 
      success: true,
      message: 'Đang xử lý đăng ký khuôn mặt cho ' + name + '. Vui lòng chờ xác nhận.'
    };
  }
