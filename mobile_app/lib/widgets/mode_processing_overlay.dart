...
class _ModeProcessingOverlayState extends State<ModeProcessingOverlay>
  ...
  late final AnimationController _p = AnimationController(
    ...
    ..repeat();
  late final AnimationController _s = AnimationController(
  ...
  CustomPainter _getModePainter() {
    ...
    switch (widget.mode) {
      case 'money':
        return _MoneyPainter(t, t2, isSpeaking: widget.isSpeaking);
      case 'caption':
        return _CaptionPainter(t, t2, isSpeaking: widget.isSpeaking);
      case 'face':
        return _FacePainter(t, t2, isSpeaking: widget.isSpeaking);
      case 'online_ocr':
        return _OnlineOCRPainter(t, t2, isSpeaking: widget.isSpeaking);
      case 'offline_ocr':
        return _OfflineOCRPainter(t, t2, isSpeaking: widget.isSpeaking);
      case 'file_read':
        return _FileReadPainter(t, t2, isSpeaking: widget.isSpeaking);
      case 'layout_analysis':
        return _LayoutPainter(t, t2, isSpeaking: widget.isSpeaking);
      default:
        return _GenericPainter(t);
    }
  }
        ...
        child: Container(
          ...
          child: AnimatedBuilder(
            ...
            builder: (context, _) => Stack(
              children: [
                // RepaintBoundary isolates custom painting from the main render tree
                Positioned.fill(
                  child: RepaintBoundary(
                    child: CustomPaint(painter: _getModePainter()),
                  ),
                ),
  ...
  Widget _card() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
            color: c.withValues(alpha: 0.3 + 0.2 * pulse), width: 1.5),
        boxShadow: [
          BoxShadow(
              color: c.withValues(alpha: 0.15 + 0.1 * pulse),
              blurRadius: 30,
              spreadRadius: 2)
        ],
      ),
      ...
      child: Row(
        ...
        children: List.generate(12, (i) {
          ...
          return AnimatedContainer(
            ...
            decoration: BoxDecoration(
              color: c.withValues(
                  alpha: 0.6 + 0.3 * sin(_wave.value * pi * 2 + i)),
              borderRadius: BorderRadius.circular(2),
            ),
          );
        }),
...
class _SpeakingWavePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Sound wave lines — reduced to 4 (from 8) for CPU efficiency
    for (int i = 0; i < 4; i++) {
  ...
  @override
  bool shouldRepaint(covariant _SpeakingWavePainter old) =>
      ...
      canvas.drawCircle(
          ...
          Paint()
            ..color = color.withValues(
                alpha: (0.3 + 0.3 * sin(t * pi * 3 + i)).clamp(0.0, 0.6)));
    }
    _corners(canvas, size, color.withValues(alpha: 0.4));
  }

  @override
  bool shouldRepaint(covariant _MoneyPainter old) => old.t != t || old.t2 != t2;
}
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      canvas.drawCircle(
          ...
          Paint()
            ..color =
                color.withValues(alpha: (0.25 * (1.0 - phase)).clamp(0.0, 0.25))
            ..style = PaintingStyle.stroke
            ..strokeWidth = 1.5);
    }
    final cp = Paint()
      ...
      ..strokeWidth = 1;
    canvas.drawLine(Offset(center.dx, center.dy - 40),
        Offset(center.dx, center.dy + 40), cp);
    canvas.drawLine(Offset(center.dx - 40, center.dy),
        Offset(center.dx + 40, center.dy), cp);
    _corners(canvas, size, color.withValues(alpha: 0.5));
  }

  @override
  bool shouldRepaint(covariant _CaptionPainter old) =>
      old.t != t || old.t2 != t2;
}
...
class _FacePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawOval(
        Rect.fromCenter(center: Offset(cx, cy), width: 120, height: 160),
        Paint()
          ..color = color.withValues(alpha: 0.15 + 0.1 * sin(t * pi * 2))
          ..style = PaintingStyle.stroke
          ..strokeWidth = 2);
    ...
    for (final f in [
      ...
      canvas.drawCircle(
          ...
          Paint()
            ..color = color.withValues(
                alpha: (0.5 * (1.0 - (scanY - f.dy) / 160)).clamp(0.1, 0.5)));
    }
    _corners(canvas, size, color.withValues(alpha: 0.4));
  }

  @override
  bool shouldRepaint(covariant _FacePainter old) => old.t != t || old.t2 != t2;
}
...
class _OnlineOCRPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRect(
        Rect.fromLTWH(0, laserY, size.width, 2),
        Paint()
          ..shader = LinearGradient(colors: [
            ...
            0.5,
            1
          ]).createShader(Rect.fromLTWH(0, laserY - 1, size.width, 2)));
    canvas.drawRect(
        Rect.fromLTWH(0, laserY - 30, size.width, 60),
        Paint()
    ...
    for (int i = 0; i < 12; i++) {
      ...
      lp.color = AppTheme.accentCyan.withValues(
          alpha: (1.0 - (laserY - y) / size.height).clamp(0.05, 0.3));
  ...
  @override
  bool shouldRepaint(covariant _OnlineOCRPainter old) =>
      old.t != t || old.t2 != t2;
}
...
class _OfflineOCRPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRect(
        Rect.fromLTWH(beamX - 1.5, 0, 3, size.height),
        Paint()
          ..shader = LinearGradient(colors: [
            Colors.transparent,
    ...
    for (int r = 0; r < 18; r++) {
      for (int c = 0; c < 10; c++) {
        final dx = size.width * (0.1 + c * 0.085),
            dy = size.height * (0.12 + r * 0.045);
        if (dx > beamX || rng.nextDouble() > 0.6) continue;
        dp.color = AppTheme.accentGreen.withValues(
            alpha: 0.1 + 0.25 * ((beamX - dx) / size.width).clamp(0.0, 1.0));
  ...
  @override
  bool shouldRepaint(covariant _OfflineOCRPainter old) =>
      old.t != t || old.t2 != t2;
}
...
class _FileReadPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 14; i++) {
      ...
      canvas.drawLine(
          Offset(xs, y),
          Offset(xs + size.width * (0.2 + rng.nextDouble() * 0.22) * prog, y),
          lp);
    }
    ...
    if (!isSpeaking) {
      ...
      canvas.drawRect(
          Rect.fromLTWH(cx - size.width * 0.25, fy - 15, size.width * 0.5, 30),
          Paint()
            ..shader = LinearGradient(
                begin: Alignment.topCenter,
                end: Alignment.bottomCenter,
                ...
                cx - size.width * 0.25, fy - 15, size.width * 0.5, 30)));
    }
  }

  @override
  bool shouldRepaint(covariant _FileReadPainter old) =>
      old.t != t || old.t2 != t2 || old.isSpeaking != isSpeaking;
}
...
class _LayoutPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final sections = [
      Rect.fromLTWH(size.width * 0.08, size.height * 0.1, size.width * 0.84,
          size.height * 0.08),
      Rect.fromLTWH(size.width * 0.08, size.height * 0.22, size.width * 0.4,
          size.height * 0.35),
      Rect.fromLTWH(size.width * 0.52, size.height * 0.22, size.width * 0.4,
          size.height * 0.15),
      Rect.fromLTWH(size.width * 0.52, size.height * 0.4, size.width * 0.4,
          size.height * 0.17),
      Rect.fromLTWH(size.width * 0.08, size.height * 0.62, size.width * 0.84,
          size.height * 0.12),
      Rect.fromLTWH(size.width * 0.08, size.height * 0.78, size.width * 0.55,
          size.height * 0.1),
    ];
    final bp = Paint()
      ..style = PaintingStyle.stroke
    ...
    for (int i = 0; i < sections.length; i++) {
      ...
      bp.color =
          c.withValues(alpha: (0.2 + 0.15 * sin(t2 * pi + i * 0.8)) * sp);
      canvas.drawRRect(rr, bp);
      if (sp > 0.5) {
        canvas.drawCircle(Offset(sections[i].left + 8, sections[i].top + 8), 3,
            Paint()..color = c.withValues(alpha: 0.6 * sp));
      }
    }
    canvas.drawRect(
        Rect.fromLTWH(0, size.height * t, size.width, 1.5),
        Paint()
          ..shader = LinearGradient(colors: [
            Colors.transparent,
            ...
            Colors.transparent
              ...
              Rect.fromLTWH(0, size.height * t - 1, size.width, 2)));
  }
...
class _GenericPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 3; i++) {
      canvas.drawCircle(
          c,
          40.0 + i * 30 + 20 * sin(t * pi * 2 + i),
          Paint()
            ..color = AppTheme.accentCyan.withValues(alpha: 0.08 - i * 0.02)
            ..style = PaintingStyle.stroke
            ..strokeWidth = 1.5);
    }
  }
  ...
  @override
  bool shouldRepaint(covariant _GenericPainter old) => old.t != t;
}
...
void _corners(Canvas canvas, Size size, Color color) {
  ...
  canvas.drawLine(
      Offset(m, size.height - m), Offset(m + l, size.height - m), p);
  canvas.drawLine(
      Offset(m, size.height - m), Offset(m, size.height - m - l), p);
