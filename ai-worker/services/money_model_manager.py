"""
Money Detection Model Manager — Quản lý riêng model nhận diện tiền Việt Nam.
...
class MoneyModelManager:
    """Singleton-style manager for money detection YOLO model."""
    ...
    def _candidates(cls) -> list[Path]:
        ...
        candidates = [
            ...
            ai_worker_dir / "models" / "money" / "onnx" / "best.onnx",
        ]
    ...
    def _can_load(path: Path) -> bool:
        ...
        if suffix == ".tflite":
            if importlib.util.find_spec("tensorflow") is None:
                print(f"[MoneyModel] Skipping TFLite (tensorflow missing): {path}")
                return False
        return True

    @classmethod
    def load(cls):
        """Lazy-load the money detection model."""
        ...
        for candidate in cls._candidates():
            ...
            if not cls._can_load(candidate):
                continue
            try:
                print(f"[MoneyModel] Loading: {candidate}")
                cls._model = YOLO(str(candidate))
                if hasattr(cls._model, "names"):
                    print(f"[MoneyModel] Classes ({len(cls._model.names)}): {list(cls._model.names.values())}")
                return cls._model
            except Exception as exc:
                errors.append(f"{candidate}: {exc}")
                print(f"[MoneyModel] Failed: {candidate}: {exc}")

        if errors:
            print("[MoneyModel] No loadable model found. Errors: " + " | ".join(errors))
        return None
    ...
    def detect(cls, image: np.ndarray) -> list[dict[str, Any]]:
        ...
        model = cls.load()
        if model is None:
            print("[MoneyModel] Model missing, skipping detection.")
            return []
    ...
    def reload(cls, model_path: str = None):
        ...
        print(f"[MoneyModel] Reloading... (path: {model_path})")
        cls._model = None
        if model_path:
            os.environ["MONEY_MODEL_PATH"] = model_path
        cls.load()
